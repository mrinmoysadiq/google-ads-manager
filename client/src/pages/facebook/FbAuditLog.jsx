...
export default function FbAuditLog() {
  ...
  const handleEditSession = (session) => {
    // Load session into fbState and go to checklist
    resetFbState()
    fbState.buyer = session.media_buyer || ''
    fbState.date = session.date
    ...
    session.answers.slice(0, QUESTION_COUNT).forEach((a, i) => {
      ...
      if (a.hasIssue) { fbState.items[i].issueText = a.note || '' }
      else { fbState.items[i].chatText = a.note || '' }
    })
    ...
    <div className="min-h-screen bg-[#1b1b1b]">
      ...
      {selectedSession && (
        <SessionModal
          ...
          onEdit={() => { handleEditSession(selectedSession); setSelectedSession(null) }}
        />
      )}
    </div>
  )
...
function TableView({ sessions, onViewSession, onEditSession }) {
              ...
              <tr key={s.id} style={{ backgroundColor: i % 2 === 0 ? '#242424' : '#2a2a2a', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => onEditSession(s)} className="text-xs font-medium hover:underline" style={{ color: '#8a8680' }}>Edit</button>
                  </div>
                </td>
              </tr>
            ))}
...
function SessionModal({ session, onClose, onEdit }) {
  return (
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          ...
          <div className="space-y-3">
            {session.answers.map((a, i) => (
              <div key={i} className="rounded-lg p-4" style={{ backgroundColor: '#2a2a2a', border: `1px solid ${a.hasIssue ? 'rgba(245,158,11,0.2)' : 'rgba(34,197,94,0.15)'}` }}>
                ...
                {a.note && (
                  <p className="text-xs whitespace-pre-wrap" style={{ color: '#8a8680' }}>{a.note}</p>
                )}
              </div>
            ))}
          </div>
